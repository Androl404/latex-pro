const popup = document.querySelector(".connection-popup"),
    ...
    popupTitle = document.querySelector(".connection-popup .title"),
...
const checkConnection = async () => {
    try {
        ...
        const response = await fetch("https://zeo.hopto.org/latex-pro/jsconfig.json");
...
const handlePopup = (status) => {
    if (status) { // If the status is true (online), update icon, title, and description accordingly
        wifiIcon.className = "bx bx-wifi";
        popupTitle.innerText = "Restored Connection";
        popupDesc.innerHTML = "Your device is now successfully connected to our servers.";
        popup.classList.add("online");
        return setTimeout(() => popup.classList.remove("show"), 2000);
    }
    // If the status is false (offline), update the icon, title, and description accordingly
    wifiIcon.className = "bx bx-wifi-off";
    popupTitle.innerText = "Lost Connection";
    popupDesc.innerHTML = "Your network is unavailable (failed request to our servers). Files cannot be saved. We will attempt to reconnect you in <span class='bold'>10</span> seconds.";
    popup.className = "connection-popup show";
    ...
    intervalId = setInterval(() => { // Set an interval to decrease the timer by 1 every second
        ...
        popup.querySelector(".desc span.bold").innerText = timer;
    }, 1000);
}
